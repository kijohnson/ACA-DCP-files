########################################################################################
########################################################################################
##### R Code for the analyses of the manuscript: Impacts of the ACA Dependent Coverage Provision on Young Adults with Cancer (Barnes)
########################################################################################
########################################################################################

#####################################################################
#### Prepare SEER DATA for analysis
#####################################################################
...
###############
## DID estimates, no multiple imputation (note that the estimates given in paper utilize multiple imputation; see below for imputation procedure; these estimates correspond to those in Table S9-S10)
###############

###### INSURANCE (Figure 2a, Supplemental Table 1)

# Insured (excluding Medicaid)
...
keep.data.ncdb <- which(data.insurance.ncdb$insurance== 0 | data.insurance.ncdb$insurance== 2) # medicaid = 1, so it's being excluded
keep.data.seer <- which(data.insurance.seer$insurance== 0 | data.insurance.seer$insurance== 2) # medicaid = 1, so it's being excluded
# Insured (including Medicaid with uninsured)
...
## Perform robust SE estimation, CI computation, parallel trends assumption testing, and estimation stratified by cancer type
...
## DCP-eligible group, Adjusted estimates
survival.ncdb.a <- coxph(Surv(survmonths,cens)~post.aca+residence+countyeducation+countyincome+sex+race+strata(cancertype),data=data.insurance.ncdb[which(data.insurance.ncdb$intervention.group==1),],x=T,robust=T)
survival.seer.a <- coxph(Surv(survmonths,cens)~post.aca+residence+countyeducation+countyincome+sex+marital+race+strata(cancertype),data=data.insurance.seer[which(data.insurance.seer$intervention.group==1),],x=T,robust=T)
ret.ncdb.a <- summary(survival.ncdb.a)$coefficients
ret.seer.a <- summary(survival.seer.a)$coefficients
c(exp(ret.ncdb.a[1,1]),exp(ci.compute(ret.ncdb.a[1,c(1,(ncol(ret.ncdb.a)-2))])),ret.ncdb.a[1,ncol(ret.ncdb.a)]) # hazard ratio estimate, 95% CI lower & upper bound, p-value
c(exp(ret.seer.a[1,1]),exp(ci.compute(ret.seer.a[1,c(1,(ncol(ret.seer.a)-2))])),ret.seer.a[1,ncol(ret.seer.a)]) # hazard ratio estimate, 95% CI lower & upper bound, p-value
## DCP-ineligble group, Adjusted estimates
survival.ncdb.a <- coxph(Surv(survmonths,cens)~post.aca+residence+countyeducation+countyincome+sex+race+strata(cancertype),data=data.insurance.ncdb[which(data.insurance.ncdb$intervention.group==0),],x=T,robust=T)
survival.seer.a <- coxph(Surv(survmonths,cens)~post.aca+residence+countyeducation+countyincome+sex+marital+race+strata(cancertype),data=data.insurance.seer[which(data.insurance.seer$intervention.group==0),],x=T,robust=T)
ret.ncdb.a <- summary(survival.ncdb.a)$coefficients
ret.seer.a <- summary(survival.seer.a)$coefficients
c(exp(ret.ncdb.a[1,1]),exp(ci.compute(ret.ncdb.a[1,c(1,(ncol(ret.ncdb.a)-2))])),ret.ncdb.a[1,ncol(ret.ncdb.a)]) # hazard ratio estimate, 95% CI lower & upper bound, p-value
c(exp(ret.seer.a[1,1]),exp(ci.compute(ret.seer.a[1,c(1,(ncol(ret.seer.a)-2))])),ret.seer.a[1,ncol(ret.seer.a)]) # hazard ratio estimate, 95% CI lower & upper bound, p-value

## Perform parallel trends assumption testing and estimation stratified by cancer type

######### Insurance and stage at diagnosis estimates stratified by DCP-eligibility (Supplementary Table 11)
data.insurance.ncdb2 <- data.insurance.ncdb[keep.data.ncdb,]
ins1.ncdb.a.eligible <- lm(insurance.resp1~post.aca+race+sex+residence+countyeducation+countyincome,data=data.insurance.ncdb2[which(data.insurance.ncdb2$intervention.group==1),])
ins1.ncdb.a.ineligble <- lm(insurance.resp1~post.aca+race+sex+residence+countyeducation+countyincome,data=data.insurance.ncdb2[which(data.insurance.ncdb2$intervention.group==0),])
## Perform robust SE estimation, CI computation, parallel trends assumption testing, and estimation stratified by cancer type
## Perform analyses for the other insurance definition (insurance.resp2; including Medicaid with uninsured) and for SEER

### Stage
estage.ncdb.a.eligible <- lm(earlystage~post.aca+race+sex+residence+countyeducation+countyincome+cancertype,data=data.han.ncdbdata.han.ncdb[which(data.han.ncdb$intervention.group==1),])
estage.ncdb.a.ineligible <- lm(earlystage~post.aca+race+sex+residence+countyeducation+countyincome+cancertype,data=data.han.ncdbdata.han.ncdb[which(data.han.ncdb$intervention.group==0),])
## Perform robust SE estimation, CI computation, parallel trends assumption testing, and estimation stratified by cancer type
## Perform analyses for late stage and for SEER
...
for(i in 1:18){
  ...
  #### The above regression models can be modified to stratify by DCP-eligibility (Supplementary Table 11). right hand side of formula would be: ~post.aca+...,data=data.han.seer[which(data.han.seer$intervention.group==1),] # instead of ~post.aca*intervention.group+...,data=data.han.seer
...
for(i in 1:18){
  ...
  #### Additional analyses/models, like for parallel trends assumption testing, or analyses stratified by cancer type, could also be added here
  #### The above regression models can be modified to stratify by DCP-eligibility (Supplementary Table 11). right hand side of formula would be: ~post.aca+...,data=data.han.ncdb[which(data.han.ncdb$intervention.group==1),] # instead of ~post.aca*intervention.group+...,data=data.han.ncdb
...
#######################################
# These analyses can be performed using the above code, overriding the data with the appropriate modified datasets as demonstrated below

##### Excluding states that expanded Medicaid early (Table S4)
data.insurance.seer <- data.insurance.seer[-which(data.insurance.seer$state %in% c("California","Connecticut","New Jersey","Washington")),]
data.han.seer <- data.han.seer[-which(data.han.seer$state %in% c("California","Connecticut","New Jersey","Washington")),]

##### Excluding states that had prior dependent coverage extensions (Table S5)
data.insurance.seer <- data.insurance.seer[-which(data.insurance.seer$state %in% c("New Jersey","New Mexico","Utah")),]
data.han.seer <- data.han.seer[-which(data.han.seer$state %in% c("New Jersey","New Mexico","Utah")),]

##### Excluding 2014 (Table S6 (SEER) and S7 (NCDB))
data.insurance.seer <- data.insurance.seer[-which(data.insurance.seer$diagnoseyear==2014),]
data.han.seer <- data.han.seer[-which(data.han.seer$diagnoseyear==2014),]
data.insurance.ncdb <- data.insurance.ncdb[-which(data.insurance.ncdb$diagnoseyear==2014),]
data.han.ncdb <- data.han.ncdb[-which(data.han.ncdb$diagnoseyear==2014),]

##### Excluding facilities with increased Medicaid coverage from 2009-2011
facsdat <- data # the raw NCDB data (OK if exclusions already applied)
facsdat <- facsdat[-which((facsdat$YEAR_OF_DIAGNOSIS %in% c(2009,2011))==F),]
facsdat <- facsdat[-which(facsdat$AGE < 27),]
facsdat$post.exp <- ifelse(facsdat$YEAR_OF_DIAGNOSIS==2011,1,0)

seer.facility <- facsdat$PUF_FACILITY_ID
seer.insurance <- facsdat$INSURANCE_STATUS
seer.insurance[which(seer.insurance %in% c(3,4))] <- 1
seer.insurance[which(seer.insurance==9)] <- NA
seer.insurance <- 3-seer.insurance 
seer.insurance[which(seer.insurance==3)] <- 0 #0=no insurance, 1 = medicaid, 2 = other insurance, 9=unk
seer.insurance2 <- seer.insurance
seer.insurance2[which(is.na(seer.insurance2))] <- 9
facsdat$insurance0 <- ifelse(seer.insurance==1,1,0) # binary indicator for medicaid (missing = NA, to be removed in analysis)
facsdat$insurance1 <- ifelse(seer.insurance2==1,1,0) # binary indicator for medicaid (missing = 9, new factor level)

facs <- unique(facsdat$PUF_FACILITY_ID)
tab <- matrix(nrow=length(facs),ncol=4)
rownames(tab) <- as.character(facs)
seer.diagnoseyear <- facsdat$YEAR_OF_DIAGNOSIS
...
for(i in facs){
  l <- l+1
  aa <- tryCatch(lm(insurance0~post.exp,data=facsdat[which(facsdat$PUF_FACILITY_ID==i),]),error=function(e) NA) # analysis with cases with missing insurance removed ***used in calculations below
  aa2 <- tryCatch(lm(insurance1~post.exp,data=facsdat[which(facsdat$PUF_FACILITY_ID==i),]),error=function(e) NA) # analysis with cases with missing insurance kept but treated as separate factor level
  results1 <- tryCatch(as.matrix(coeftest(aa, vcov.=vcovHC)),error=function(e) NA)
  results2 <- tryCatch(as.matrix(coeftest(aa2, vcov.=vcovHC)),error=function(e) NA)
  tab[l,c(1,2)] <- tryCatch(results1[nrow(results1),c(1,4)],error=function(e) NA) # pulls the estimate (column 1) and p-value (from column 4 to column 2) of the change in % with medicaid from 2009 to 2011
  tab[l,c(3,4)] <- tryCatch(results2[nrow(results2),c(1,4)],error=function(e) NA)
}

removers <- which(is.na(tab[,1]) | is.na(tab[,2]) | (tab[,1]>0) )
facs.removers <- rownames(tab)[removers]
data <- data[-which(data$PUF_FACILITY_ID %in% facs.removers),]
...
p1<-ggplot(test_ins, aes(x=y, y=xseery, color=Population))+ 
  ...
  theme_minimal() +
  theme(text=element_text(family="Arial"),legend.position="none",axis.text.y=element_text(family="Arial",face=ifelse(levels(test_ins$xseery)=="Overall","bold","plain"),
...
p2<-ggplot(test_estage, aes(x=y, y=xseery, color=Population))+ 
  ...
  scale_x_continuous(limits=c(-40,+42)) +
  theme_minimal() +
  theme(text=element_text(family="Arial"),legend.position="none", axis.text.y=element_text(family="Arial",face=ifelse(levels(test_estage$xseery)=="Overall","bold","plain"),
...
p3<-ggplot(test_lstage, aes(x=y, y=xseery, color=Population))+ 
  ...
  scale_x_continuous(limits=c(-40,+42)) +
  theme_minimal() +
  theme(text=element_text(family="Arial"),legend.position="bottom", axis.text.y=element_text(family="Arial",face=ifelse(levels(test_lstage$xseery)=="Overall","bold","plain"),
